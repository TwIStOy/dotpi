...
import { readSubagentStatuslineBridge } from "./bridges.js";

type AgentAsciiColor = "red" | "green" | "yellow" | "blue" | "magenta" | "cyan";

const AGENT_ASCII_COLOR_SEQUENCE: AgentAsciiColor[] = [
  "magenta",
  "green",
  "blue",
  "cyan",
...
function normalizeAgentAsciiColor(
  value: string | undefined,
): AgentAsciiColor | undefined {
...
function fallbackAgentAsciiColor(name: string): AgentAsciiColor {
  ...
  return (
    AGENT_ASCII_COLOR_SEQUENCE[hash % AGENT_ASCII_COLOR_SEQUENCE.length] ??
    "magenta"
  );
}
...
export function subagentStatuslineMarker(
  cwd: string,
  maxInnerWidth = 24,
): { plain: string; styled: string } | undefined {
  const bridgeInfo = readSubagentStatuslineBridge()?.getCurrentSubagent(cwd);
  const envName = process.env.PI_SUBAGENT_CHILD_AGENT?.trim();
  const rawName = bridgeInfo?.name?.trim() || envName;
  if (!rawName) return undefined;
  const color =
    normalizeAgentAsciiColor(bridgeInfo?.color) ??
    normalizeAgentAsciiColor(process.env.PI_SUBAGENT_CHILD_COLOR) ??
    fallbackAgentAsciiColor(rawName);
