 ...
 *   TaskExecute  — Execute tasks as subagents
 *
 * Commands:
...
The task tools haven't been used recently. If you're working on tasks that would benefit using TaskCreate to add more tasks to track progress, consider using TaskUpdate to mark tasks as in_progress when starting work and completed when when done). Consider cleaning up the task list if this is relevant to the current work. Make sure that you NEVER mention this reminder to the user
</system-reminder>`;

export default function initTasks(pi: ExtensionAPI) {
  ...
  /** Spawn a subagent via pi.events RPC. */
  ...
  /** Stop a subagent via pi.events RPC. */
    ...
    const unsub = pi.events.on(`subagents:rpc:ping:reply:${requestId}`, (raw: unknown) => {
      ...
      if (remoteVersion === undefined) {
        ...
        pendingWarning =
          ...
          `subagents has v${remoteVersion}) — please update for task execution support.`;
      } else if (remoteVersion < PROTOCOL_VERSION) {
        pendingWarning =
          `Subagents extension is outdated (protocol v${remoteVersion}, ` +
  ...
  // Failure → store error, revert to pending, don't cascade
  pi.events.on("subagents:failed", (data) => {
    const { id, error, status } = data as { id: string; error?: string; result?: string; status: string };
    const taskId = agentTaskMap.get(id);
    if (!taskId) return;
    agentTaskMap.delete(id);
    const task = store.get(taskId);
    ...
    if (status === "stopped") {
      store.update(task.id, { status: "completed", metadata: { ...task.metadata, result: task.metadata?.result } });
      autoClear.trackCompletion(task.id, currentTurn);
    } else {
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      if (!subagentsAvailable) {
        return textResult(
          ...
          "Ensure the subagents extension is loaded and supports the RPC protocol."
        );
      }
