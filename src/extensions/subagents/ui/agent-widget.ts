...
function extractToolLabel(toolName: string, args: any): string {
  ...
  switch (toolName) {
    case "read":
      return args.path
        ? `reading ${shortenPath(String(args.path))}`
        : "reading";
    case "bash":
      if (args.command) {
        const cmd = String(args.command).split("\n")[0].trim();
        return cmd.length > 50
          ? `running ${cmd.slice(0, 50)}…`
          : `running ${cmd}`;
      }
      return "running command";
    case "edit":
      return args.path
        ? `editing ${shortenPath(String(args.path))}`
        : "editing";
    case "write":
      return args.path
        ? `writing ${shortenPath(String(args.path))}`
        : "writing";
    case "grep":
      return args.pattern
        ? `searching /${String(args.pattern).slice(0, 30)}/`
        : "searching";
    case "find":
      return args.pattern
        ? `finding *${String(args.pattern).slice(0, 30)}*`
        : "finding files";
    case "ls":
      return args.path
        ? `listing ${shortenPath(String(args.path))}`
        : "listing";
    default:
      return TOOL_DISPLAY[toolName] ?? toolName;
  }
  ...
  private renderWidget(tui: any, theme: Theme): string[] {
    ...
    for (const a of running) {
      ...
      runningLines.push([
        truncate(
          theme.fg("dim", "├─") +
            ` ${theme.fg("accent", frame)} ${theme.bold(name)} ${theme.fg("muted", a.description.slice(0, 30))} ${theme.fg("dim", "·")} ${theme.fg("dim", statsText)}`,
        ),
  ...
  return {
    onToolActivity: (act: ToolActivity) => {
      if (act.type === "start") {
        ...
        activity.activeTools.set(act.toolCallId ?? act.toolName, {
          name: act.toolName,
          label,
        });
      } else {
        if (act.toolCallId) {
          ...
          for (const [k, v] of activity.activeTools) {
            if (v.name === act.toolName) {
              activity.activeTools.delete(k);
              break;
            }
          }
        }
