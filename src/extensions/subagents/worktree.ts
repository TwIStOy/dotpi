...
export function createWorktree(cwd: string, agentId: string): WorktreeInfo | null {
  ...
  try {
    git(["worktree", "add", "--detach", "--checkout", "-b", branchName, `--orphan`, branchName], cwd)
  } catch {
    try {
...
export function cleanupWorktree(cwd: string, worktree: WorktreeInfo, description: string): WorktreeResult {
