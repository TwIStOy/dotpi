...
async function showRunningAgents(
  ...
  if (recent.length > 0) {
    for (const record of recent) {
      ...
      const desc = record.description.slice(0, 40);
      options.push(
        `${icon} ${record.type} (${record.id.slice(0, 8)}) ${desc} [${record.status}]`,
      );
    }
  }
...
async function showAgentTypes(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const allTypes = [
    ...new Set([...types, ...getDefaultAgentNames(), ...getUserAgentNames()]),
  ];
  ...
  const choice = await ctx.ui.select(
    "Agent Types  • project  ◦ global  ◆ extension  · default",
    options,
  );
  if (!choice) return;
...
async function showAgentTypeDetail(
  ctx: ExtensionCommandContext,
  name: string,
): Promise<void> {
  const config = getAgentConfig(name);
...
function agentConfigToFrontmatter(config: AgentConfig): string {
  ...
  if (config.displayName && config.displayName !== config.name)
    lines.push(`display_name: ${config.displayName}`);
  lines.push(`description: ${config.description}`);
  if (config.builtinToolNames?.length)
    lines.push(`tools: ${config.builtinToolNames.join(", ")}`);
  if (config.disallowedTools?.length)
    lines.push(`disallowed_tools: ${config.disallowedTools.join(", ")}`);
  if (config.extensions !== true)
    lines.push(
      `extensions: ${config.extensions === false ? "none" : (config.extensions as string[]).join(", ")}`,
    );
  if (config.skills !== true)
    lines.push(
      `skills: ${config.skills === false ? "none" : (config.skills as string[]).join(", ")}`,
    );
...
async function ejectAgent(
  ...
  const scope = await ctx.ui.select("Eject to:", [
    "Project (.pi/agents/)",
...
async function toggleAgent(
  ...
  if (hasFile) {
    ...
    const scope = await ctx.ui.select("Create disabled override in:", [
      "Project (.pi/agents/)",
      "Global (~/.pi/agent/agents/)",
      "Cancel",
    ]);
    ...
    const content = agentConfigToFrontmatter(config).replace(
      "enabled: true",
      "enabled: false",
    );
    writeFileSync(filePath, content, "utf-8");
...
async function showSettings(
  ...
  const options = [`Max concurrent: ${manager.getMaxConcurrent()}`, "Back"];
...
async function showScheduledJobs(
  ctx: ExtensionCommandContext,
...
async function createScheduledJob(
  ...
  const scheduleTypes = [
    "once (one-shot at ISO datetime)",
    "interval (every N minutes)",
    "cron (cron expression)",
  ];
  const scheduleTypeChoice = await ctx.ui.select(
    "Schedule type",
    scheduleTypes,
  );
  ...
  if (scheduleTypeChoice.startsWith("once")) {
    scheduleType = "once";
    const defaultTime = new Date(Date.now() + 5 * 60_000)
      .toISOString()
      .slice(0, 19);
    schedule =
      (await ctx.ui.input("Run at (ISO datetime):", defaultTime)) ?? "";
    if (!schedule) return;
    try {
...
export function registerAgentsCommand(
  ...
  pi.registerCommand("agents", {
    description:
      "Manage subagents — view running agents, types, schedules, and settings",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      if (!ctx.hasUI) return;

      const sections = ["Running & Recent Agents", "Agent Types"];
      if (scheduler) {
