export type PiActivitySeverity =
  | "debug"
  | "info"
  | "success"
  | "warning"
  | "error";
export type PiActivityImportance =
  | "critical"
  | "important"
  | "normal"
  | "noisy";
...
const ACTIVITY_BROKER_SYMBOL = Symbol.for("dotpi.pi.activity");
...
export function buildQuestionActivity(
  event: QuestionActivityEvent,
): PiActivityEvent {
  const header =
...
function activityBroker(): PiActivityBroker | undefined {
  const broker = (globalThis as unknown as Record<PropertyKey, unknown>)[
    ACTIVITY_BROKER_SYMBOL
  ];
  return broker &&
    typeof broker === "object" &&
    typeof (broker as PiActivityBroker).publish === "function"
