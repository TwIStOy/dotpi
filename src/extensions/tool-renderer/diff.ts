...
const DIFF_HIGHLIGHT_MAX_CHARS = 180_000; // eslint-disable-line @typescript-eslint/no-unused-vars
...
function highlightDiffContent(
  ...
  try {
    ...
    const lines = Array.isArray(highlighted)
      ? highlighted
      : String(highlighted).replace(/\r\n/g, "\n").split("\n");
...
function wordTokens(text: string): WordToken[] {
  ...
  while ((match = re.exec(text)))
    tokens.push({ end: re.lastIndex, start: match.index, text: match[0] });
  return tokens;
}
...
function wordDiffRanges(oldText: string, newText: string): WordDiffRanges {
  ...
  const table = new Uint16Array(
    (oldTokens.length + 1) * (newTokens.length + 1),
  );
...
function applyFullLineBg(
  ...
  const reapplied = text.replace(/\x1b\[[0-9;]*m/g, (code) =>
    sgrClearsBackground(code) ? `${code}${open}` : code,
  );
...
function styleAnsiVisibleRanges(
  ...
  if (!hasAnsi(text)) {
    const base = (value: string) =>
      maybeBg(theme, baseBgToken, theme.fg(fgToken, value), useBackground);
    const highlight = (value: string) =>
      maybeBg(theme, highlightBgToken, theme.fg(fgToken, value), useBackground);
  ...
  function inHighlightRange(index: number): boolean {
    while (rangeIndex < sorted.length && index >= sorted[rangeIndex]![1])
      rangeIndex++;
...
function diffOps(
  ...
  for (let i = 0; i < start; i++)
    ops.push({ text: oldLines[i] ?? "", type: "ctx" });
...
export function hiddenDiffLine(count: number): StructuredDiffLine {
  return {
    content:
      count > 0 ? `… ${count} unchanged line${count === 1 ? "" : "s"} …` : "…",
    newNum: null,
    oldNum: null,
    type: "sep",
  };
...
function hiddenHunksAfter(
  allRows: StructuredDiffLine[],
  shownRows: StructuredDiffLine[],
): number {
  const shown = new Set(
    shownRows
      .map((line) => line.hunk)
...
function compactStructuredDiffLines(
  lines: StructuredDiffLine[],
  contextLines = DIFF_CONTEXT_LINES,
): StructuredDiffLine[] {
  const changed = lines
    .map((line, index) =>
      line.type === "add" || line.type === "del" ? index : -1,
    )
...
function formatNum(value: number | null, width: number): string {
  return value === null
    ? " ".repeat(width)
    : `${" ".repeat(Math.max(0, width - String(value).length))}${value}`;
}
...
function lineWordRanges(
  ...
  if (
    !(
      ...
      (line.type === "add" && mate.type === "del")
    )
  )
    return [];
  const oldText = diffDisplayContent(
    line.type === "del" ? line.content : mate.content,
  );
  const newText = diffDisplayContent(
    line.type === "add" ? line.content : mate.content,
  );
...
function pairDiffRows(rows: StructuredDiffLine[]): Array<{
  ...
  while (index < rows.length) {
    ...
    const count = Math.max(dels.length, adds.length);
    for (let i = 0; i < count; i++)
      paired.push({ left: dels[i] ?? null, right: adds[i] ?? null });
  }
  return paired;
}
...
function shouldUseSplitDiff(
  ...
  for (const line of rows) {
    ...
    contentLines++;
    if (visibleLength(diffDisplayContent(line.content)) > codeWidth)
      wrapCandidates++;
  }
...
function renderSplitDiff(
  ...
  const innerWidth = Math.max(
    2,
    tableWidth -
      visibleLength(leftBorder) -
      visibleLength(divider) -
      visibleLength(rightBorder),
  );
...
function displayUnifiedDiffPath(
  path: string,
  oldPath?: string,
  newPath?: string,
): string {
...
export function parseUnifiedDiffOutput(
  output: string,
    ...
    if (current.sawHunk && (current.additions > 0 || current.removals > 0)) {
      files.push({
        diff: {
          ...
          chars: current.chars || output.length,
          hunks: current.hunkCount || countStructuredHunks(current.lines),
          lines: current.lines,
          path: displayUnifiedDiffPath(
            current.path,
            current.oldPath,
            current.newPath,
          ),
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith("diff --git ")) {
      const paths = splitGitHeaderPaths(line.slice("diff --git ".length)).map(
        cleanDiffPath,
      );
    ...
    if (current && line.startsWith("+++ ")) {
      ...
      current.path = displayUnifiedDiffPath(
        current.path,
        current.oldPath,
        current.newPath,
      );
    ...
    if (
      current.sawHunk &&
      current.oldHunkEnd !== null &&
      current.newHunkEnd !== null
    ) {
      const hidden = Math.max(
        hunk.oldStart - current.oldHunkEnd - 1,
        hunk.newStart - current.newHunkEnd - 1,
      );
    ...
    while (
      index < lines.length &&
      (oldConsumed < hunk.oldCount || newConsumed < hunk.newCount)
    ) {
...
export function renderBashDiffOutput(
  output: string,
  theme: any,
  expanded: boolean,
  cwd?: string,
  ...
  const totalAdditions = files.reduce(
    (sum, file) => sum + file.diff.additions,
    0,
  );
  const totalRemovals = files.reduce(
    (sum, file) => sum + file.diff.removals,
    0,
  );
  const totalLines = files.reduce(
    (sum, file) => sum + file.diff.lines.length,
    0,
  );
  ...
  for (const file of files) {
    ...
    if (remainingRows !== null)
      remainingRows -= Math.min(
        ...
        fileLimit ?? file.diff.lines.length,
      );
  }
  ...
  if (hiddenFiles > 0)
    rendered.push(
      theme.fg(
        "muted",
        `… ${hiddenFiles} more file diff${hiddenFiles === 1 ? "" : "s"}${expanded ? ` · UI cap ${Math.max(0, renderedFiles)}/${files.length}` : " · ctrl+o to expand"}`,
      ),
    );
  else if (
    remainingRows !== null &&
...
export function readTextForDiff(
  ...
  try {
    ...
    return Buffer.byteLength(text, "utf8") <= MAX_DIFF_INPUT_BYTES
      ? text
      : undefined;
  } catch {
    return undefined;
...
export function attachDiffDetails(
  result: any,
