...
export const CORE_TOOL_RENDERERS = new Set([
  "read",
  "bash",
  "grep",
  "find",
...
export function shouldUseGenericRenderer(name: string): boolean {
  if (!name || CORE_TOOL_RENDERERS.has(name) || name === "apply_patch")
    return false;
...
export function isUnknownToolComponent(component: any): boolean {
  return (
    component?.toolDefinition === undefined &&
    component?.builtInToolDefinition === undefined
  );
}

export function shouldUseUnknownToolRenderer(
  component: any,
  name: string,
): boolean {
  return (
...
export function componentDefinesRenderer(
  component: any,
...
function parsePatchBodyLine(rawLine: string): {
  ...
  if (marker === "+" || marker === "-" || marker === " ")
    return { content: rawLine.slice(1), marker };
...
function parseApplyPatchUpdateDiff(lines: string[]): StructuredDiff {
  ...
  for (const rawLine of lines) {
    ...
    if (rawLine.startsWith("@@")) {
      if (
        ...
        diffLines.push(hiddenDiffLine(0));
  ...
  return {
    additions,
    chars,
    hunks: Math.max(hunk, numbered.hunks),
...
function parseApplyPatchPreview(patchText: string): ApplyPatchChange[] {
  ...
  while (index < lines.length) {
    ...
    changes.push({
      diff,
      displayPath: moveTo ? `${path} → ${moveTo}` : path,
      kind,
...
function applyPatchSummaryTarget(
  changes: ApplyPatchChange[],
  theme: any,
): string {
  if (changes.length === 0) return theme.fg("muted", "patch");
  if (changes.length > 1)
    return theme.fg("muted", `${changes.length} files changed`);
...
function applyPatchResultSummary(
  changes: ApplyPatchChange[],
...
export function renderApplyPatchCall(
  ...
  const files =
    changes.length > 0
      ? changes.map((change) => change.moveTo || change.path)
      : extractApplyPatchFiles(patchText);
  const multiFile = changes.length > 1 || files.length > 1;
  const summary = multiFile
    ? theme.fg(
        "muted",
        `${Math.max(changes.length, files.length)} files changed`,
      )
...
function summarizeScheduleWakeupCall(args: any, theme: any): string {
  const delay = formatScheduleWakeupDelay(
    args?.delaySeconds ?? args?.delay_seconds ?? args?.delay ?? args?.seconds,
  );
  const reason =
    stringArg(args, "reason", "prompt", "description") || "scheduled wakeup";
...
function summarizeGenericCall(name: string, args: any, theme: any): string {
  if (isMcpToolName(name)) {
    ...
    return arg
      ? `${theme.fg("accent", label)} ${theme.fg("muted", oneLine(arg, 48))}`
  ...
  switch (name) {
    ...
    case "fetch_content": {
      const url =
        stringArg(args, "url") || stringArrayArg(args, "urls")[0] || "fetch";
    ...
    case "Agent":
      return theme.fg(
        "accent",
        oneLine(stringArg(args, "description", "prompt") || "launch agent", 72),
      );
    case "TaskCreate":
      return theme.fg(
        "accent",
        oneLine(stringArg(args, "subject", "description") || "create task", 72),
      );
    case "TaskGet":
    case "TaskUpdate":
    case "TaskOutput":
    case "TaskStop":
      return theme.fg("accent", stringArg(args, "taskId", "task_id") || "task");
    case "TaskList":
      return theme.fg("muted", "task list");
    case "TaskExecute": {
      const ids = stringArrayArg(args, "taskIds", "task_ids");
      return ids.length <= 1
        ? theme.fg("accent", ids[0] ?? "start tasks")
        : `${theme.fg("accent", ids[0]!)}${theme.fg("muted", ` +${ids.length - 1} tasks`)}`;
    }
    case "ScheduleWakeup":
      return summarizeScheduleWakeupCall(args, theme);
    default:
      return theme.fg(
        "accent",
        oneLine(
...
export function renderGenericToolCall(
  ...
  return makeTruncatedLines(
    `${genericStatusPrefix(context, theme)}${toolLabel(theme, `${humanizeToolName(name)} `)}${summarizeGenericCall(name, args, theme)}`,
  );
}
...
function unknownToolStatus(
  ...
  if (
    ...
    new RegExp(`\\b${escapedName}\\b.*not found`, "i").test(raw)
  )
    return theme.fg("error", "x not found");
...
export function renderUnknownToolCall(
  ...
  if (context?.executionStarted) return makeEmpty();
  return makeTruncatedLines(
    `${genericStatusPrefix(context, theme)}${toolLabel(theme, `${humanizeToolName(name)} `)}${summarizeUnknownToolCall(name, args, theme)}`,
  );
}

export function renderUnknownToolResult(
  name: string,
  result: any,
  { expanded, isPartial }: any,
  theme: any,
...
export function renderScheduleWakeupCall(
  args: any,
  theme: any,
  context: any,
): TruncatedLines | ReturnType<typeof makeEmpty> {
...
export function renderScheduleWakeupResult(
  result: any,
  { expanded, isPartial }: any,
  theme: any,
  context: any,
): TruncatedLines | ReturnType<typeof makeEmpty> {
  return renderUnknownToolResult(
    "ScheduleWakeup",
    result,
    { expanded, isPartial },
    theme,
...
export function renderGenericToolResult(
  ...
  if (context?.isError) {
    ...
    return makeTruncatedLines(
      `${treeConnector(theme, "└")}${theme.fg("error", first)}`,
    );
  }
  if (lines.length === 0)
    return makeTruncatedLines(
      `${treeConnector(theme, "└")}${theme.fg("success", "done")}`,
    );
  if (lines.length === 1)
    return makeTruncatedLines(
      `${treeConnector(theme, "└")}${theme.fg("muted", oneLine(lines[0]!, 120))}`,
    );
