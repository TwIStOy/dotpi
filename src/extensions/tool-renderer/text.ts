...
export function bashCallText(args: any, theme: any, _cwd?: string): string {
  ...
  const command =
    rawCommand.length > max ? `${rawCommand.slice(0, max - 1)}…` : rawCommand;
  ...
  const styledContinuation = continuationLines
...
export function isGitDiffCommand(command: unknown): boolean {
  ...
  return /(?:^|[;&|()]\s*)(?:(?:env\s+(?:-\S+\s+)*(?:[A-Za-z_][A-Za-z0-9_]*=\S+\s+)*)|(?:command\s+))*git(?:\s+(?!--?diff\b)(?:-[A-Za-z]\S*|--\S+)(?:\s+(?!diff(?:\s|$))\S+)*)*\s+diff(?:\s|$)/.test(
    normalized,
  );
}
