...
export function trimOuterBlankLinesAroundRules(lines: string[]): string[] {
  ...
  return isHorizontalRuleLine(trimmed[0]) &&
    isHorizontalRuleLine(trimmed[trimmed.length - 1])
    ? trimmed
    : lines;
}
...
export function trimThinkingOnlyAssistantLines(lines: string[]): string[] {
  ...
  if (lines[0]?.includes(zoneStart) && !trimmed[0]?.includes(zoneStart))
    trimmed[0] = `${zoneStart}${trimmed[0] ?? ""}`;
  return trimmed;
}
...
export function ansiPartsFromStyled(styled: string): AnsiParts {
  ...
  return {
    ...
    close: styled.slice(markerIndex + marker.length),
  };
}

export function fgParts(
  theme: any,
  token: string,
  ...
  try {
    ...
    return {
      open: styled.slice(0, index),
      close: styled.slice(index + marker.length),
    };
  } catch {
    return { open: "", close: "" };
  }
