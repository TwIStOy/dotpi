...
function capBatchItemText(
  ...
  if (maxBytes <= markerBytes + 128 || maxLines <= 3)
    return { text: fitPrefix(marker, maxBytes), truncated: true };
...
function capBatchItemsForAggregate(items: BatchToolItem[]): BatchToolItem[] {
  if (
    utf8Length(toolBatchOutput(items)) <= TOOL_BATCH_MAX_OUTPUT_BYTES &&
    lineCount(toolBatchOutput(items)) <= TOOL_BATCH_MAX_OUTPUT_LINES
  )
    return items;
  ...
  const availableBytes = Math.max(
    1,
    TOOL_BATCH_MAX_OUTPUT_BYTES - overheadBytes - 512,
  );
  const availableLines = Math.max(
    1,
    TOOL_BATCH_MAX_OUTPUT_LINES - overheadLines - items.length,
  );
  const byteBudgets = allocateBatchBudgets(
    items.map((item) => utf8Length(item.resultText)),
    availableBytes,
  );
  const lineBudgets = allocateBatchBudgets(
    items.map((item) => lineCount(item.resultText)),
    availableLines,
  );

  return items.map((item, index) => {
    const capped = capBatchItemText(
      item.resultText,
      byteBudgets[index] ?? 1,
      lineBudgets[index] ?? 1,
    );
    return capped.truncated
      ? { ...item, resultText: capped.text, truncated: true }
      : item;
  });
}
  ...
  properties: {
    calls: {
      ...
      items: {
        ...
        description:
          "One tool call. Prefer { tool, args } (e.g. { tool: 'read', args: { path: 'README.md' } }). MCP-style { name, arguments } and flat shorthand { tool: 'read', path: 'README.md' } are also accepted. The 'tool' / 'name' field must be one of read | grep | find | ls | bash.",
        properties: {
          tool: {
            type: "string",
            enum: ["read", "grep", "find", "ls", "bash"],
            description:
              "Tool to run inside the batch (or pass 'name' instead).",
          },
          name: {
            type: "string",
            enum: ["read", "grep", "find", "ls", "bash"],
            description: "Alias for 'tool' (MCP-style shape).",
          },
          args: {
            type: "object",
            additionalProperties: true,
            description:
              "Arguments for the selected tool. Optional; flat sibling fields are folded into args.",
          },
          arguments: {
            type: "object",
            additionalProperties: true,
            description: "Alias for 'args' (MCP-style shape).",
          },
        },
      },
    },
    concurrency: {
      type: "number",
      description:
        "Maximum calls to run at once. Defaults to all calls, capped by settings.",
    },
  },
  required: ["calls"],
} as const;
...
function normalizeBatchCalls(value: unknown): BatchToolCall[] {
  ...
  for (const raw of value) {
    ...
    const nestedArgs =
      nestedArgsRaw &&
      typeof nestedArgsRaw === "object" &&
      !Array.isArray(nestedArgsRaw)
        ? nestedArgsRaw
        : {};
...
async function mapBatchWithConcurrency<TIn, TOut>(
  items: TIn[],
  concurrency: number,
  ...
  const workers = new Array(
    Math.max(1, Math.min(concurrency, items.length || 1)),
  )
    .fill(null)
    .map(async () => {
      while (true) {
...
function renderToolBatchText(
  ...
  const header =
    ...
    theme.fg(
      ...
      failed > 0 ? ` · ${failed} failed` : " · succeeded",
    ) +
...
function toolBatchOutput(items: BatchToolItem[]): string {
  ...
  for (const item of items) {
    ...
    lines.push(
      "",
      `## ${label}`,
      item.isError ? "Status: failed" : "Status: completed",
      item.resultText || "(no output)",
    );
  }
  return lines.join("\n");
}
...
function renderToolBatchCallText(args: any, theme: any, cwd?: string): string {
  ...
  const lines = [
    stackPrefix(theme) +
      toolLabel(
        theme,
        `Batch ${calls.length || 0} tool${calls.length === 1 ? "" : "s"} launching`,
      ),
  ];
...
export function registerToolBatch(
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Batch 2+ independent read/search/list/diagnostic bash calls into one compact result.",
    promptGuidelines: [
    ...
    async execute(
      ...
      if (calls.length === 0)
        return {
          content: [{ type: "text", text: "No valid calls provided." }],
          details: { failed: 0, items: [], succeeded: 0, total: 0 },
        };
      if (calls.length > maxCalls) {
        return {
          content: [
            {
              type: "text",
              text: `Too many calls (${calls.length}). Max is ${maxCalls}.`,
            },
          ],
          details: {
            failed: calls.length,
      ...
      const concurrency = Math.max(
        ...
        Math.min(
          calls.length,
          Math.floor(Number(params?.concurrency) || calls.length),
          maxCalls,
        ),
      );
      ...
      const items = await mapBatchWithConcurrency(
        calls,
        concurrency,
        async (call, index): Promise<BatchToolItem> => {
          ...
          try {
            ...
            if (!original?.execute)
              throw new Error(`Built-in tool unavailable: ${call.tool}`);
      ...
      const details: BatchToolDetails = {
        failed,
        items: cappedItems,
        succeeded: cappedItems.length - failed,
        total: cappedItems.length,
      };
      return {
        content: [{ type: "text", text: toolBatchOutput(cappedItems) }],
        details,
        isError: failed > 0,
      };
    },
    ...
    renderResult(
      ...
      if (!details?.items)
        return makeTruncatedLines(textContent(result) || "(no output)");
      return makeTruncatedLines(
        renderToolBatchText(
          details.items,
          theme,
