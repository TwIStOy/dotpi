...
function markBashStarted(context: any): BashLiveTailState {
  ...
  if (!state.startedAt && context?.executionStarted)
...
function scheduleBashLiveTailRerender(
  state: BashLiveTailState,
  context: any,
  delayMs: number,
): void {
  if (
...
export function registerRead(pi: ExtensionAPI, agent: any, cwd: string): void {
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      return getBuiltInTool(agent, contextCwd(context, cwd), "read").execute(
        id,
        params,
        signal,
        onUpdate,
...
export function registerBash(pi: ExtensionAPI, agent: any, cwd: string): void {
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      return getBuiltInTool(agent, contextCwd(context, cwd), "bash").execute(
        id,
        params,
        signal,
        onUpdate,
...
export function registerEdit(pi: ExtensionAPI, agent: any, cwd: string): void {
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      const result = await getBuiltInTool(agent, effectiveCwd, "edit").execute(
        id,
        params,
        signal,
        onUpdate,
      );
      const after = result?.isError
        ? before
        : readTextForDiff(targetPath, effectiveCwd);
      return attachDiffDetails(
        result,
        before,
        after,
    ...
    renderCall(args: any, theme: any, context: any) {
      ...
      if (context?.argsComplete) {
        ...
        const previewComponent = renderMutationCallPreview(
          "Edit",
          String(targetPath),
          diffs,
          theme,
...
export function registerWrite(pi: ExtensionAPI, agent: any, cwd: string): void {
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      const targetPath = params?.path ?? params?.file_path;
      const before = readTextForDiff(targetPath, effectiveCwd);
      const result = await getBuiltInTool(agent, effectiveCwd, "write").execute(
        id,
        params,
        signal,
        onUpdate,
    ...
    renderCall(args: any, theme: any, context: any) {
      ...
      if (context?.argsComplete && typeof args?.content === "string") {
        ...
        const label: "Write" | "Create" =
          before === undefined ? "Create" : "Write";
        const diff = {
          ...buildStructuredDiff(before ?? "", args.content),
...
export function registerReadOnly(
  pi: ExtensionAPI,
  agent: any,
  cwd: string,
  ...
  pi.registerTool({
    ...
    async execute(
      ...
      return getBuiltInTool(agent, contextCwd(context, cwd), toolName).execute(
        id,
        params,
        signal,
        onUpdate,
