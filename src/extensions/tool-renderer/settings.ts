...
function piSettingsPaths(cwd = process.cwd()): string[] {
  const userDir = resolve(
    expandHome(process.env.PI_CODING_AGENT_DIR?.trim() || "~/.pi/agent"),
  );
...
export function settingEnum<T extends string>(
  key: string,
  allowed: readonly T[],
  fallback: T,
  cwd?: string,
...
export function bashOutputMode(cwd?: string): BashOutputMode {
  return settingEnum(
    "bashOutputMode",
    ["hidden", "summary", "opencode", "preview"] as const,
    "opencode",
    cwd,
  );
}
...
export function bashLiveOutputDelayMs(cwd?: string): number {
  return Math.max(
    0,
    Math.floor(settingNumber("bashLiveOutputDelayMs", 1000, cwd)),
  );
}
...
export function toolChromeMode(cwd?: string): ToolChromeMode {
  return settingEnum(
    "toolChrome",
    ["off", "transparent", "outlines"] as const,
    "outlines",
    cwd,
  );
}
